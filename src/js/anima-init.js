const AnimaInitiative = (() => {
  ...
  const HE = (() => {
    ...
    const re = new RegExp(
      `(${Object.keys(entities).map(esRE).join("|")})`,
      "g",
    );
  ...
  const _h = {
    ...
    attr: {
      ...
      char: (o, c) =>
        `${ch("@")}${ch("{")}${c || "CHARACTER NAME"}${ch("|")}${o}${ch("}")}`,
    },
    bold: (...o) => `<b>${o.join(" ")}</b>`,
    italic: (...o) => `<i>${o.join(" ")}</i>`,
    font: {
  ...
  const getFumbleSpan = function (fumbled, roll) {
    if (fumbled) {
      return (
        ...
        getFumblePenalty(fumbled, roll).toString() +
        "</span> [fumble]"
      );
    }
    return "";
  ...
  const formatDieRoll = function (rollData, character) {
    ...
    let openRollData = autoOpenRoll(
      rollData.rolls[0].rolls[0],
      characterSuccess,
    );
    let newTotal =
      parseFloat(rollData.total) +
      openRollData.bonus -
      getFumblePenalty(critFail, rollData.rolls[0].rolls[0]);
      ...
      function (m, s) {
        var type =
          "gmlayer" === s.token.get("layer")
            ...
            : (s.character &&
                _.filter(
                  s.character.get("controlledby").split(/,/),
                  function (c) {
            ...
            : "npc";
        if (
          "graphic" !== s.token.get("type") ||
          "token" !== s.token.get("subtype")
        ) {
          return m;
        }
        m[type].push(
          '<div style="float: left;display: inline-block;border: 1px solid #888;border-radius:5px; padding: 1px 3px;background-color:' +
            ...
            ((s.token && s.token.get("name")) ||
              (s.character && s.character.get("name")) ||
              "(Creature)") +
            "</div>" +
            "<div>" +
  ...
  const helpParts = {
    ...
    helpCommands: (/*context*/) =>
      _h.section(
        ...
        _h.inset(
          ...
          _h.paragraph(
            `This command displays the help and configuration options.`,
          ),
        ),
      ),
              ...
              (s) =>
                `${_h.ui.float(
                  s === state[ScriptName].config.sortOption
                    ...
                    : _h.ui.button(
                        `Use ${s}`,
                        `!anima-init-config --sort-option|${s}`,
                      ),
    ...
    autoOpenInitConfig: (/* context */) =>
      _h.section(
        ...
        _h.paragraph(
          `This option causes AnimaInitiative to open the Turn Order whenever it makes an initiative roll.`,
        ),
        _h.inset(
          _h.paragraph(
            `${_h.ui.float(
              ...
              (a) =>
                `${_h.ui.float(
                  a === state[ScriptName].config.announcer
                    ...
                    : _h.ui.button(
                        `Use ${a}`,
                        `!anima-init-config --set-announcer|${a}`,
                      ),
    ...
    helpConfig: (context) =>
      _h.outer(_h.title(ScriptName, Version), helpParts.configuration(context)),
  ...
  const showHelp = (playerid) => {
    const who = (getObj("player", playerid) || { get: () => "API" }).get(
      "_displayname",
    );
    let context = {
      who,
  ...
  const findCharacterFumble = (character) => {
    return character
      ? parseInt(getAttrByName(character.id, CharacterFailAttribute))
  ...
  const findInitiativeBonus = (character) => {
    ...
    var bonus =
      stat && !Number.isNaN(Number(stat)) ? parseFloat(stat) : undefined;
    if (bonus) {
      return bonus;
    }
  ...
  const makeRollsForIDs = (ids, options = {}) => {
    ...
    let pageid = (rollSetup[0] || { token: { get: () => {} } }).token.get(
      "pageid",
    );
    ...
    let finalize = _.after(initRolls.length, function () {
      ...
      if (
        state[ScriptName].config.autoOpenInit &&
        !Campaign().get("initativepage")
      ) {
        Campaign().set({
        ...
        function (msg) {
          ...
          rdata.bonus =
            rdata.bonus % 1 === 0
              ? rdata.bonus
              : parseFloat(
                  rdata.bonus.toFixed(state[ScriptName].config.maxDecimal),
                );
  ...
  const handleInput = (msg_orig) => {
    var msg = _.clone(msg_orig),
      ...
      manualBonusMin = 0,
      isReroll = false;
    const who = (getObj("player", msg.playerid) || { get: () => "API" }).get(
      "_displayname",
    );
          ...
          switch (cmds[0]) {
            ...
            case "adjust":
              ...
              if (cmds[1] && cmds[1].match(/^[-+]?\d+(\.\d+)?$/)) {
                ...
                manualBonusMin = _.isNaN(manualBonusMin)
                  ? -10000
                  : manualBonusMin;
                  ...
                  turnorder: JSON.stringify(
                    _.map(
                      ...
                      function (e) {
                        if ("-1" !== e.id) {
                          e.pr = Math.max(
                            (_.isNaN(parseFloat(e.pr)) ? 0 : parseFloat(e.pr)) +
                              manualBonus,
                            manualBonusMin,
            ...
            case "clear":
              ...
              Campaign().set({
                turnorder: "[]",
                initiativepage: state[ScriptName].config.autoOpenInit
                  ? false
          ...
          switch (opt.shift()) {
            case "sort-option":
              if (sorters[opt[0]]) {
                ...
                omsg =
                  "<div><b>Error:</b> Not a valid sort method: " +
                  opt[0] +
                  "</div>";
              }
              sendChat(
            ...
            case "set-max-decimal":
              if (opt[0].match(/^\d+$/)) {
                ...
                omsg =
                  "<div><b>Error:</b> Not a valid decimal count: " +
                  opt[0] +
                  "</div>";
              }
              sendChat(
            ...
            case "toggle-replace-roll":
              state[ScriptName].config.replaceRoll =
                !state[ScriptName].config.replaceRoll;
              sendChat(
                "",
                `/w "${who}" ` +
            ...
            case "set-announcer":
              if (announcers[opt[0]]) {
                ...
                omsg =
                  "<div><b>Error:</b> Not a valid announcer: " +
                  opt[0] +
                  "</div>";
              }
              sendChat(
